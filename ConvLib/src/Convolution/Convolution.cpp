...
void apply_convolution(const std::string &filename, int k_width, int k_height,
  ...
  auto get_pixel_value = [&](int x, int y, int channel) -> float {
    ...
    if (bit_count == 32) {
      ...
      uint32_t pixel =
          *reinterpret_cast<uint32_t *>(&image.data[index * 3]) & 0xFFFFFF;
      return (float)((pixel >> (channel * 8)) & 0xFF) / 255.0f;
    }
  ...
  auto set_pixel_value = [&](int x, int y, int channel, float value) {
    int index = y * width + x;
    uint8_t byteValue =
        static_cast<uint8_t>(clamp(value * 255.0f, 0.0f, 255.0f));

    if (bit_count == 32) {
      uint32_t &pixel = *reinterpret_cast<uint32_t *>(&output.data[index * 4]);
      pixel = (pixel & ~(0xFF << (channel * 8))) | (byteValue << (channel * 8));
    } else if (bit_count == 24) {
      uint32_t &pixel = *reinterpret_cast<uint32_t *>(&output.data[index * 3]);
      pixel = (pixel & ~(0xFF << (channel * 8))) | (byteValue << (channel * 8));
    } else if (bit_count == 16) {
